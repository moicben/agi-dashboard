...
export const TEMPMAIL_STATE_API_URL = '/api/temp-mail/state';
export const TEMPMAIL_INBOXES_API_URL = '/api/temp-mail/inboxes';
export const TEMPMAIL_REFRESH_INBOX_API_URL = '/api/temp-mail/refresh-inbox';
export const TEMPMAIL_REFRESH_ALL_API_URL = '/api/temp-mail/refresh-all';
export const TEMPMAIL_MARK_READ_API_URL = '/api/temp-mail/mark-read';
...
export async function fetchTempMailState() {
    const response = await fetch(TEMPMAIL_STATE_API_URL, { cache: 'no-store' });
    const data = await parseJsonResponse(response);
    return { inboxes: data?.inboxes || [], emails: data?.emails || [] };
}

export async function createTempMailInbox(input) {
    const response = await fetch(TEMPMAIL_INBOXES_API_URL, {
        method: 'POST',
...
export async function deleteTempMailInbox(inboxId) {
    const params = new URLSearchParams();
    params.set('id', String(inboxId ?? ''));
    const response = await fetch(`${TEMPMAIL_INBOXES_API_URL}?${params.toString()}`, { method: 'DELETE' });
    const data = await parseJsonResponse(response);
    return Boolean(data?.ok);
}

export async function refreshTempMailInbox(inboxId) {
    const response = await fetch(TEMPMAIL_REFRESH_INBOX_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const data = await parseJsonResponse(response);
    return {
        newEmailsCount: Number(data?.newEmailsCount ?? 0),
        inserted: Array.isArray(data?.inserted) ? data.inserted : []
    };
}

export async function refreshTempMailAll() {
    const response = await fetch(TEMPMAIL_REFRESH_ALL_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
    });
...
export async function markTempMailRead(emailId) {
    const response = await fetch(TEMPMAIL_MARK_READ_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
